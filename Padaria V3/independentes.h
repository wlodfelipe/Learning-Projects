...
void ler_txt(estoque *vetor, int quantidade_produtos)
{
    ...
    // essa parte comentada seria para ler o arquivo binário
    /*
    ponteiro_ler = fopen("archivo.bin", "rb");
    if (ponteiro_ler == NULL)
    {
        printf("Erro ao abrir um arquivo!\n");
        exit(1);
    ...
    fread(&auxbin, sizeof(int), 1, ponteiro_ler);
    fgetc(ponteiro_ler); // Ler o caractere de nova linha '\n'

    for (int i = 0; i < quantidade_produtos; i++)
    {
        fread(&vetor[i].codigo_item, sizeof(int), 1, ponteiro_ler);
        fgetc(ponteiro_ler); // Ler o caractere de nova linha '\n'
        fgets(vetor[i].nome_item, 50, ponteiro_ler);
        vetor[i].nome_item[strcspn(vetor[i].nome_item, "\n")] = '\0'; // Remover o caractere de nova linha '\n'
        fread(&vetor[i].preco_item, sizeof(int), 1, ponteiro_ler);
        fgetc(ponteiro_ler); // Ler o caractere de nova linha '\n'
        fread(&vetor[i].quantidade_vendida, sizeof(int), 1, ponteiro_ler);
        fgetc(ponteiro_ler); // Ler o caractere de nova linha '\n'
        fread(&vetor[i].quantidade_item, sizeof(int), 1, ponteiro_ler);
        fgetc(ponteiro_ler); // Ler o caractere de nova linha '\n'
    }
    */
}
...
int descobrir_tamanho() // só do produtos.txt
{
...
estoque *aloca_vetor(int tamanho) // existe por causa do verificador de erro 
{
    estoque *vetor;
